...
def solve_ode(lambda_, u0, a, b, c):
    t_span = (0, 1500)
    t_eval = np.arange(1400, 1500, 0.25)
    sol = solve_ivp(rossler, t_span, u0, vectorized=True, args=(lambda_,a,b,c), t_eval=t_eval)
...
if __name__ == "__main__":
    ...
    for nr in range(n_realizations):
        ...
        # model heterogeneity
        a,b,c = 0.1, 0.1, 18
        a = np.random.uniform(a*0.9, a*1.1)
        b = np.random.uniform(b*0.9, b*1.1)
        c = np.random.uniform(c*0.9, c*1.1)
        
        with mp.Pool(processes=28) as pool:
            results = pool.starmap(solve_ode, [(lambdas[i], u0.flatten(), a,b,c) for i in range(length)])
    ...
    np.savez("./data/NCM/heteroParams_errdict3.npz", **clusters_errdict)
